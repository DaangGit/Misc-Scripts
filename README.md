...
def create_question(question, alt_question=None, options=None, input_type='text'):
    st.markdown(f"<h4>{question}</h4>", unsafe_allow_html=True)
    if options is None:
        if input_type == 'text':
            answer = st.text_input(f'Enter your {question}')
        elif input_type == 'number':
            answer = st.number_input(f'Enter your {question}')
        elif input_type == 'yesno':
            answer = st.radio(f'{question}', ['Yes', 'No'])
       
    else:
        
        if input_type == 'multiple':
            answer = st.multiselect(f'{question}', options)
        else:
            answer = st.selectbox(f'{question}', options)
    return answer
...
st.markdown(f"<h2>Client Information</h2>", unsafe_allow_html=True)
...
st.markdown(f"<h2>Investment Objectives</h2>", unsafe_allow_html=True)
...
st.markdown(f"<h2>Liquidity Needs</h2>", unsafe_allow_html=True)
...
if liquidity_needs == 'Yes':
    ...
    annual_withdrawal_percentage = create_question('What % of the accountâ€™s value will you need annually?', options=annual_withdrawal_options)

    withdrawal_time_options = ['Immediately', '1 â€“ 3 Years', '3 â€“ 5 Years', '5 â€“ 10 Years', '10 â€“ 15 Years', 'Over 15 Years']
...
st.markdown(f"<h2>Other Preferences</h2>", unsafe_allow_html=True)
...
# Questions for Gamblerâ€™s Fallacy
